...
public class LayerTwoManager implements LayerTwoService {
    ...
    public boolean installFirewallRule(IpAddress srcIpAddress, IpAddress dstIpAddress, PortNumber dstPort) {
		...
		int portNumberAsInt = (int) dstPort.toLong();
        for (Device d : devices) {
			...
			TrafficSelector selector = DefaultTrafficSelector.builder()
					...
					.matchTcpDst(TpPort.tpPort(portNumberAsInt)) 
					.build();
            ...
            flowRuleService.applyFlowRules(firewallRule);
        }
        return true;
    }
    ...
    private class LayerTwoPacketProcessor implements PacketProcessor {
        ...
        public void forward(PacketContext pc) {
            ...
            // get the macTable from current deviceId, create a new MacTableEntry, put the srcMac and corresponding macTableEntry in it.
            Map<MacAddress, MacTableEntry> currentMacTable = macTables.get(cp.deviceId());
            MacTableEntry macTableEntry = new MacTableEntry(cp.port(), Duration.ofSeconds(60));
            currentMacTable.putIfAbsent(srcMac, macTableEntry);
            ...
            MacTableEntry dstMacEntry = currentMacTable.get(dstMac);
            if (dstMacEntry != null){
